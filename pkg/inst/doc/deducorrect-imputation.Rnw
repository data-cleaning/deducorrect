...
returned by {\sf deductiveLevels} in Figure \ref{RdeductiveLevels}. The record
...
record with these values for $v_1$, $v_2$ and $v_3$. In such cases case {\sf
deductiveLevels} returns {\sf NULL} by default, since it is detected that no
feasible records exist. If the feasibility check is turned off by passing {\sf
checkFeasibility=FALSE}, the fact that there are multiple solutions
($v_4\in\{{\rm a,b}\}$) yields a zero-length character vector.
...
v <- c(v1='c',v2='b',v3=NA,v4=NA)
deductiveLevels(E,v)
v <- c(v1=NA,v2=NA,v3=NA,v4='b')
deductiveLevels(E,v)
v <- c(v1='b',v2='c',v3='a',v4=NA)
deductiveLevels(E,v)
deductiveLevels(E,v,checkFeasibility=FALSE)
@
\caption{Deriving deductive imputations using {\sf deductiveLevels}.}
