...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section[{\bf NEW!} Deductive imputation methods]{Deductive imputation methods}
In certain cases missing values in data records can be derived from the
observed values and the editrules. Methods which use the edit rules to derive
imputation values from observed values are called {\em deductive imputation}
methods.

Deductive imputation methods share the important assumption that all observed
values are correct, so that imputing the missing data can be done consistently
without violating any (derived) edits.

The rest of this section starts with examples of using the most user-friendly
deductive imputation functions of the package. After this we describe some
underlying functions exposed to user space and give a bit of background.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Deductive imputation with {\sf deduImpute}}
...
figure from one page on a paper form to another\footnote{In spite of the availability of
web-based forms, some respondents still prefer paper forms.}.  
%
%
\begin{Rcode}
<<keep.source=TRUE>>=
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Deductive imputation with {\sf solSpace}}
Consider a numerical record $\bf x$ with some values observed and some values missing.
We will write ${\bf x}=({\bf x}_{\rm obs},{\bf x}_{\rm miss})$, with $x_{\rm obs}$ the observed
values and $x_{\sf miss}$ the missing ones. Supposing further that $\bf x$ must
obey a set of equality restrictions
\begin{equation}
    {\bf Ax}={\bf b}, 
\end{equation}
with ${\bf A}$ a constant matrix and {\bf b} a constant vector, we may write
${\bf A}=[{\bf A}_{\rm obs},{\bf A}_{\rm miss}]$. Consequently, we have
\begin{equation}
...
\end{equation}
with {\bf z} an arbitrary real vector of the right dimension and 
...
(s <- solSpace(E,x))
@
The top two and bottom two rows of {\bf C} have zero coefficients, yielding a
unique solution for those values of ${\bf x}_{\rm miss}$.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Deductive imputation with {\sf deductiveZeros}}
