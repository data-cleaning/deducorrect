...
\begin{tikzpicture}[node distance = 2cm, auto, decoration={brace,amplitude=10pt}]
    ...
    \node [block, below of=deduimp1]   (errloc)    {error localization};
    ...
    \draw [decorate, line width=1pt] ($(deducorr.north) + (7,0)$) -- ($(deduimp1.south) + (7,0)$)
...
Deductive imputation appears twice in the process flow chart of Figure
\ref{flowchart}. It appears right after performing deductive corrections, since
this allows the deductive imputation routines to use better quality observed
values to base the imputation values on.  After this, the smallest (weighted)
number of variables to adapt or impute can be determined using error
