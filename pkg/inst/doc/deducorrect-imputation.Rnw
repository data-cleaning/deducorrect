...
\maketitle
\begin{abstract}
Numerical and categorical data used for statistical analyses is often
plagued with missing values and inconsistencies. In many cases, a number
of missing values may be derived, based on the consistency rules imposed
on the data and the observed values in a record. The methods used for
such derivations are called {\em deductive imputation}. In this paper,
we describe the newly developed deductive imputation functionality of
{\sf R} package {\sf deducorrect}. The package gained methods to deductively
impute numerical as well as categorical data and integrates closely with
the {\sf editrules} package. Methods on setting up a partial data editing
...
Before introducing our example, we note that in general, an edit $e$ can be
...
$v_2={\rm a}$, so this may be imputed with certaintly. Finally, the record
...
s$corrections
@
\caption{Deductive imputations for categorical data using {\sf deduImpute}.}
...
the set of edits. This process is repeated untill all missing values are
treated.
...
consistently by just imuting the {\em chromosome} variable. If we choose
...
@
However, one must be carefull since, as shown above, the result may be an
...
\section{Conclusions}
In this paper we demonstrated the newly developed deductive imputation
facilities offered by the deducorrect package. From version 1.1, {\sf
deducorrect} offers a toolbox for automated deductive correction of numerical
data and deductive imputation of numerical and categorical data, integrating
closely with the {\sf editrules} package of the same authors.
